...
namespace Wormhole.Mod
{
    ...
    public class JumpDriveLogic : MyGameLogicComponent
    {
        ...
        public override void UpdateOnceBeforeFrame()
        {
            ...
            MyAPIGateway.TerminalControls.AddControl<IMyJumpDrive>(MyAPIGateway.TerminalControls.CreateControl<IMyTerminalControlSeparator, IMyJumpDrive>("WormholeSeparator"));
            var listBox = MyAPIGateway.TerminalControls.CreateControl<IMyTerminalControlListbox, IMyJumpDrive>("WormholeDestinationsListBox");
        ...
        private static bool Visible(IMyTerminalBlock terminalBlock)
        {
            ...
            return JumpComponent.Instance.JdDefinitionIds.Any(b => b == ((MyCubeBlock)terminalBlock).BlockDefinition.Id);
        }
        ...
        private static void ItemSelected(IMyTerminalBlock block, List<MyTerminalControlListBoxItem> selected)
        {
            ...
            MyAPIGateway.Multiplayer.SendMessageToServer(DestinationSelectNetId, MyAPIGateway.Utilities.SerializeToBinary(
                new ItemSelectedMessage
                {
        ...
        {
            foreach (var gate in JumpComponent.Instance.Gates.Where(gate => new BoundingSphereD(gate.Value.Position, gate.Value.Size).Intersects(block.CubeGrid.WorldAABB)))
            {
                items.AddRange(gate.Value.Destinations
                     .Select(destination => new MyTerminalControlListBoxItem(MyStringId.GetOrCompute(destination.DisplayName), MyStringId.NullOrEmpty, destination.Id)));
            }
        }

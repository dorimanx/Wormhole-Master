...
namespace Wormhole
{
    public class Plugin : TorchPluginBase, IWpfPlugin
    {
        ...
        public void WormholeTransferOut(string sendTo, WormholeGate wormholeGate, BoundingSphereD gate)
        {
            foreach (var grid in MyAPIGateway.Entities.GetTopMostEntitiesInSphere(ref gate).OfType<IMyCubeGrid>())
            {
                ...
                if (gts != null)
                {
                    ...
                    {
                        WormholeTransferOutFile(sendTo, grid, WormholeDrive, wormholeGate, WormholeDrives);
                    }
                }
            }
        }
        ...
        private void WormholeTransferOutFile(string sendTo, IMyCubeGrid grid, IMyJumpDrive wormholeDrive,
            WormholeGate gate, IEnumerable<IMyJumpDrive> wormholeDrives)
        {
            ...
            if (playerInCharge?.Identity is null ||
				!wormholeDrive.HasPlayerAccess(playerInCharge.Identity.IdentityId) ||
            ...
            (Config.GateVisuals
                ? _jumpManager.StartJump(gate, (MyPlayer)playerInCharge, (MyCubeGrid)wormholeDrive.CubeGrid)
                ...
                () =>
                {
                    ...
                    if (Config.WormholeGates.FirstOrDefault(s => s.Name.Trim() == destName) is { } internalWormhole)
                    {
                        ...
                        _clientEffectsManager.NotifyJumpStatusChanged(JumpStatus.Perform, gate, (MyCubeGrid)grid, freePos);
                    ...
                    else
                    {
                        ...
                        _clientEffectsManager.NotifyJumpStatusChanged(JumpStatus.Perform, gate, (MyCubeGrid)grid);
                ...
                })).ContinueWith(_ => _clientEffectsManager.NotifyJumpStatusChanged(JumpStatus.Succeeded, gate, (MyCubeGrid)grid));
        }
        ...
        private static void OnGridSpawned(MyEntity entity, IDictionary<long, MyObjectBuilder_Character> savedCharacters)
        {
                ...
                .OrderBy(static b => b is MyCryoChamber ? 1 : 0))
            {
                ...
                RefreshUtil.EntityRefresh.RefreshAroundGrid(cockpit.CubeGrid);
            }
        }
